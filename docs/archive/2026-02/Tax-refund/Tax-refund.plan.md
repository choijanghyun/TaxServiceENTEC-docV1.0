...
| 세법기준 입력자료 | `refer-to-doc/경정청구-입력자료-세법기준-정리.md` | 공제/감면별 입력자료 세법 근거 |
...
| Database | PostgreSQL 15 | JSON 컬럼, 부분 유니크 인덱스, REF_* 기준정보 |
...
│   - PostgreSQL 15 + REF_* 기준정보 26개 테이블                   │
└─────────────────────────────────────────────────────────────────┘
```
...
| **원본 불변성** | INP_RAW_DATASET INSERT ONLY, 수정 시 새 req_id |
| **요약 재생성** | SMR_* = INP_RAW_DATASET에서 언제든 재파싱 |
| **결과 이중 보관** | OUT_* 정규 + OUT_REPORT_JSON 직렬화 |
| **기준정보 독립** | REF_* = req_id 없이 연도별 독립 관리 |
...
**63개 테이블** (5개 주제영역, schema.md 기준)

| 주제영역 | 접두어 | CORP | INC | 공통 | 합계 | 역할 |
|---------|--------|:----:|:---:|:----:|:----:|------|
| 입력 보관 | INP_ | 6 | 4 | 10 | **20** | JSON 원본 + 파싱 데이터 |
| 중간 요약 | SMR_ | - | - | 7 | **7** | 계산용 파생 데이터 |
| 산출 결과 | OUT_ | 1 | 2 | 6 | **9** | 결과 이중 보관 |
| 감사 로그 | AUD_ | - | - | 1 | **1** | APPEND ONLY 이력 |
| 기준 정보 | REF_ | 7 | 4 | 15 | **26** | 세율/규칙 독립 관리 |

**테이블 명명 규칙**: `{주제영역}_{세목구분(S/C/I)}_{테이블명}`
- 예: `INP_RAW_DATASET`, `REF_C_INVESTMENT_CREDIT_RATE`, `OUT_I_LOSS_CARRYBACK`
...
├── US-023: 기준정보 관리 (REF_* 26개 테이블)    [8 SP]
...
├── US-028: 지방소득세 환급액 안내 (보고서 포함)      [3 SP]
...
| 리스크 | 영향 | 가능성 | 대응 방안 |
|--------|:----:|:-----:|-----------|
| 연간 세법 개정 (12월 말) | 높음 | 매우높음 | REF_* 분리 설계, 개정 후 2주 내 반영 |
...
| 1.4 | 2026-02-18 | P4 범위 수정 — P4-01~12→P4-01~13 (P4-13 전자신고 세액공제 반영) | Plan (PDCA) |
