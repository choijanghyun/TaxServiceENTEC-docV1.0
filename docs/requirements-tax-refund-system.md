...
> **Status**: Draft
> **Related Documents**:
> - 개발설계서: `참고1번-development-design-v2.md` (v2.1)
> - 법인세 프롬프트: `법인세-프롬프트_v2.0.md`
> - 종합소득세 프롬프트: `종합소득세-프롬프트_v2.0.md`
> - 업계 리서치: `참고2번-industry-research-tax-refund-programs.md`
> - 세법기준 입력자료: `docs/경정청구-입력자료-세법기준-정리.md`
...
**역할**: 세율 테이블, 상호배제 규칙, 업종 코드 등 기준정보(RF_* 26개 테이블) 유지보수
...
| EP-03 | 공제/감면 계산 엔진 | 77개 점검항목(법인 40개 + 개인 37개) 자동 산출, 53개 계산 공식 적용 | P-01 |
| EP-04 | 최적 조합 탐색 | 상호배제 10규칙 적용, Branch & Bound 알고리즘, 최저한세/농특세 적용 | P-01 |
| EP-05 | 보고서 생성 및 제출 | 환급액 비교표, 근거 설명, 경정청구서 초안 3종 생성 | P-01, P-02 |
| EP-06 | 기준정보 관리 | 세율/공제율/상호배제 규칙 등 RF_* 26개 테이블 연도별 버전 관리 | P-03 |
...
- 개인(INC): 사업장별 소재지를 각각 개별 판단한다. 다사업장 입력 시 각 사업장별 소재지 코드를 기준정보(RF_S_CAPITAL_ZONE)와 교차 검증한다
...
**인수 조건**:
- 10개 규칙을 모두 자동 체크한다 (RF_S_MUTUAL_EXCLUSION 기준정보 참조):
...
- R&D 세액공제 최저한세 배제율 테이블(RF_C_RD_MIN_TAX_EXEMPT)을 연도별로 자동 참조한다
...
**인수 조건**:
- 기준정보는 RF_* 26개 테이블에 과세연도 기준으로 이력 관리된다
...
- 기준정보(RF_* 테이블) 수정은 ADMIN 역할만 가능하다
...
| US-021 | 환급가산금 자동 계산 | EP-05 | 고객 편의 증대 | Phase 1 완료 |
...
| 공식 정확도 | 53개 공식 단위 테스트 100% 통과 | JUnit5 단위 테스트 |
...
| 원본 데이터 보호 | RI_S_RAW_DATA INSERT ONLY 정책 (물리적 삭제 불가) | DB 트리거 + 감사 로그 |
| 감사 로그 | 모든 계산 이력 AL_S_CALCULATION에 보관 | 로그 완전성 테스트 |
...
| 계산 재현성 | 동일 req_id에 대해 동일 결과를 언제든 재산출 가능 (SV_*는 RI_S_RAW_DATA에서 재파싱) |
...
| 기준정보 갱신 | RF_* 테이블 연도별 버전 관리로 세법 개정 반영 |
...
| 원본 데이터 | RI_S_RAW_DATA INSERT ONLY (변경 시 신규 버전 생성 필수) |
...
| 문서 | 경로 | 설명 |
|------|------|------|
| 개발설계서 v2.1 | `참고1번-development-design-v2.md` | 기술 아키텍처 및 모듈 설계 |
| 법인세 프롬프트 | `법인세-프롬프트_v2.0.md` | 40개 점검항목 상세 |
| 종합소득세 프롬프트 | `종합소득세-프롬프트_v2.0.md` | 37개 점검항목 상세 |
| 업계 리서치 | `참고2번-industry-research-tax-refund-programs.md` | 경쟁사 분석 |
| 세법기준 입력자료 | `docs/경정청구-입력자료-세법기준-정리.md` | 입력 항목별 세법 근거 |
